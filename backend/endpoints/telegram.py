...
@router.get("/telegram/status", summary="Get Telegram client status", response_model=TelegramStatusResponse, operation_id="get_telegram_status")
...
@router.delete("/telegram/messages", summary="Clear all Telegram messages", response_model=TelegramClearResponse, operation_id="clear_telegram_messages")
async def clear_telegram_messages(telegram_manager=Depends(get_telegram_manager)) -> TelegramClearResponse:
    try:
        return TelegramClearResponse(success=True, messages_deleted=await clear_messages())
    except Exception as e:
